 ...
 * 从请求头提取 Bearer Token → 调用 service-auth /validate → 校验租户有效性 → 注入用户、租户和权限上下文头到下游。
 */
@Component
public class JwtAuthFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        return authClient.get()
                ...
                .flatMap(response -> {
                    ...
                    return validateTenant(result.getTenantId())
                            .flatMap(tenantActive -> {
                                if (!tenantActive) {
                ...
                .onErrorResume(e -> {
                    log.error("Gateway security validation failed", e);
                    exchange.getResponse().setStatusCode(HttpStatus.SERVICE_UNAVAILABLE);
                    return exchange.getResponse().setComplete();
                });
    }

    private Mono<Boolean> validateTenant(String tenantId) {
    ...
    private record TenantValidationResult(String tenantId,
                                          String tenantName,
                                          String status,
                                          boolean active,
