...
本变更横跨 `service-auth` 数据模型/API 和 Vue 菜单工作台，目标是彻底退出历史 button 节点并统一到 Page Capability 发布链路。
...
- 通过前向迁移清理历史 button 行，动态路由仅消费导航节点。

**Non-Goals:**

- 本变更不改变角色授权草稿、编译和发布协议。
- 本变更不把搜索、刷新、打开抽屉等本地 UI 行为注册为可授权能力。
- 本变更删除历史 button 菜单行，且不新增第二套按钮授权事实表。
...
Legacy button menu rows are not part of the target model. A forward-only cleanup migration removes them after authorization facts have moved to `sys_auth_grant`; the menu API rejects new button nodes, and page-bearing menus must bind a registered `pageCode`.

All Owner page declarations are composed into one `SYSTEM_MANAGEMENT` catalog version. The current catalog lookup contract selects one active catalog, so separate active catalogs per service would make pages disappear depending on selection order. Each page carries explicit Owner metadata and explicit registered targets; composition changes packaging, not business ownership.
...
历史 button 节点由前向迁移直接清理，菜单工作台和菜单写接口均不再提供此模型。真实授权事实继续由 `sys_auth_grant` 保存。
...
- [现有 button 节点与目录能力名称不一致] → V85 直接清理 button 行，授权事实保留在 `sys_auth_grant`。
...
2. 扩展菜单实体、请求/响应和服务保存逻辑，页面型菜单缺少 `pageCode` 时拒绝保存。
...
- 无遗留清理事项；V85 已完成历史 button 节点物理清理。
