...
if ($Rebuild -or -not (Test-Path $firstJar)) {
    ...
    $serviceList = "trio-base-services/service-auth,trio-base-services/service-org,trio-base-services/service-lowcode,trio-base-services/service-workflow-engine,trio-base-platform/platform-gateway"
...
# ── Step 3: 并行启动 JAR ────────────────────────────
Write-Host "[START] 并行启动 5 个服务..." -ForegroundColor Green
foreach ($svc in $services) {
