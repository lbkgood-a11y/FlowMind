...
- 写操作加 `@Transactional`
- 先校验（参数合法性 + 业务唯一性），再写入
- 角色基础信息接口不写授权；功能授权必须通过 Authz API 写入 `sys_auth_grant`
...
export namespace SystemRoleApi {
  export interface RoleDetail extends SystemRole {
    // Read-only visible menu projection derived from sys_auth_grant.
    menuIds: string[];
  }
}
...
3. 在 `api/` 下添加 API 函数
4. 后端：迁移文件添加菜单、`sys_auth_resource`、`sys_auth_action` → Controller 添加 `@RequirePermission`
5. 后端：迁移文件通过 `sys_auth_grant` 给 ADMIN 自动授权
