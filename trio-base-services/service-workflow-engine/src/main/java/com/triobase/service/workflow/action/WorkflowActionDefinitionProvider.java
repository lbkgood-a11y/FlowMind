...
public class WorkflowActionDefinitionProvider implements ActionDefinitionProvider {
    ...
    private ActionDefinition processStart() {
        ...
        definition.getRequiredGuards().add(ActionGuardRequirement.of(OWNER_SERVICE, "PROCESS_INSTANCE_STARTABLE", "流程实例可启动"));
    ...
    private ActionDefinition taskAction(String actionType, String permission) {
        ...
        definition.getDefaultRefreshScopes().addAll(List.of("document", "list", "actions", "timeline", "workflow"));
        definition.getRequiredGuards().add(ActionGuardRequirement.of(OWNER_SERVICE, "PROCESS_TASK_ACTIONABLE", "流程任务当前可执行"));
        definition.setPayloadSchemaJson("""
                {
                  "type": "object",
                  "required": ["taskId"],
    ...
    private ActionDefinition closureAction(String actionType, String permission) {
        ...
        definition.getDefaultRefreshScopes().addAll(List.of("document", "list", "actions", "timeline", "workflow"));
        definition.getRequiredGuards().add(ActionGuardRequirement.of(OWNER_SERVICE, "PROCESS_CLOSURE_EFFECT_ACTIONABLE", "闭环补偿项当前可处理"));
        definition.setPayloadSchemaJson("""
                {
                  "type": "object",
                  "required": ["effectId"],
