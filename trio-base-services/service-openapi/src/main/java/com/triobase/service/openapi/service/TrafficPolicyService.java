...
public class TrafficPolicyService {
 ...
 @Transactional public TrafficPolicyVersion updateDraft(String id,CreateTrafficPolicyRequest r){validate(r);TrafficPolicyVersion v=require(id);if(v.getLifecycleState()!=VersionLifecycleState.DRAFT)throw new BizException(40990,"OPENAPI_POLICY_NOT_DRAFT");String tenant=targetTenant(r.tenantId());if(mapper.selectCount(new LambdaQueryWrapper<TrafficPolicyVersion>().eq(TrafficPolicyVersion::getEnvironment,r.environment()).eq(TrafficPolicyVersion::getScopeType,r.scopeType()).eq(TrafficPolicyVersion::getScopeId,r.scopeId()).eq(TrafficPolicyVersion::getLifecycleState,VersionLifecycleState.DRAFT).ne(TrafficPolicyVersion::getId,id))>0)throw new BizException(40990,"OPENAPI_POLICY_DRAFT_EXISTS");v.setTenantId(tenant);v.setEnvironment(r.environment());v.setScopeType(r.scopeType());v.setScopeId(r.scopeId());v.setPolicyContent(r.policyContent().deepCopy());v.setContentHash(hash(v.getPolicyContent()));touch(v);mapper.updateById(v);return v;}
