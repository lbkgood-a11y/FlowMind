...
1. 进入菜单管理并新增一级菜单或下级菜单。
2. 填写显示名称、上级、排序等导航信息。
3. 对承载业务页面的菜单，从“业务页面”下拉列表选择后端已经注册的页面；系统保存稳定的 `pageCode`。
4. 在菜单详情的“页面功能”区域检查后端返回的页面访问、数据读取和可授权操作。
5. 确认需要配置的操作均为 `READY`；其他状态应根据后端诊断说明先修复，不能直接授权。
6. 进入角色管理并选择目标角色，在页面功能工作台中勾选需要的业务操作并发布授权。
7. 保存后重新登录或刷新动态路由，并使用真实账号验收菜单和按钮。

注意：

- 可授权按钮和页面操作由 Owner 服务发布到 Page Capability Catalog，实施人员不得手工创建按钮权限或填写 API 权限码。
- 菜单只绑定页面并描述导航；页面功能目录是可授权操作的权威来源，最终授权事实仍写入 `sys_auth_grant`。
- 绑定页面不代表用户已经获得页面内的查看、编辑、审批权限，仍需在角色授权中选择并发布对应功能。
- 父级目录可以因子菜单可见而自动展示，不需要创建额外功能授权。
- 历史 button 菜单节点只用于兼容查看，不应作为新增权限配置入口。

页面功能完整性状态：

| 状态 | 含义 | 实施操作 |
|---|---|---|
| `READY` | 页面声明、后端 Target 和依赖完整 | 可以授权 |
| `PARTIAL` | 部分映射缺失 | 根据诊断补齐后再授权 |
| `BROKEN` | 后端保护或目录关系异常 | 禁止授权并联系 Owner 修复 |
| `UNMAPPED` | 尚未映射真实受保护动作 | 禁止授权 |
