  ...
  <Page title="OpenAPI 生命周期总览" description="从契约设计到应用开放、策略生效和运行监控的闭环状态">
    <BusinessPageScaffold pattern="multi-table">
      ...
      <MultiTableLayout :columns="2">
        ...
        <Card class="section" title="阻断项">
          <Space wrap>
            <Tag v-for="item in readiness?.blockers" :key="item" color="orange">{{ item }}</Tag>
            <Tag v-if="readiness?.ready" color="green">生命周期资产已就绪</Tag>
          </Space>
        </Card>
      </MultiTableLayout>
      <Row :gutter="[12,12]">
